...
// Add this require statement for X.ai Grok integration. Ensure you have run 'npm install openai'.
...
const GROK_MODEL_NAME = "grok-4-fast-non-reasoning"; // The model to use when --grok flag is active
...
const useGrok = process.argv.includes('--grok'); // Changed from --groq to --grok
...
const grokApiKey = process.env.XAI_API_KEY; // Changed from GROQ_API_KEY
if (useGrok && !grokApiKey) {
    console.error("FATAL: --grok flag is set, but GROK_API_KEY environment variable is not set.");
    process.exit(1);
}
...
if (useGrok) {
    grokClient = new OpenAI({
        ...
        timeout: 360000, // Long timeout for reasoning models
    });
}
    ...
    for (const modelName of modelNames) {
        try {
            ...
            if (error.message.includes('403') || error.message.toLowerCase().includes('forbidden')) {
                console.log(`Retrying with the next model in the list...`);
                continue;
            }
            break;
        }
...
async function generateWithGrok(systemPrompt, userPrompt) {
    console.log(`Attempting to use X.ai Grok model: ${GROK_MODEL_NAME}...`);
    try {
        ...
        console.log("Successfully generated content with Grok.");
        return {
            response: {
                text,
            },
...
function removeBoilerplate(text) {
    ...
    const patternsToRemove = [
        /your browser is deprecated.*?upgrade/gi,
        /sorry, youtube music is not optimized for your browser/gi,
        /check for updates or try google chrome/gi,
        /get chrome/gi,
        /sign in/gi,
        /privacy policy/gi,
        /terms of service/gi,
    ...
    return cleanedText.replace(/\s{2,}/g, ' ').replace(/^\s*[\r\n]/gm, '').trim();
}
...
async function processOriginalImage(imageUrl) {
    ...
    try {
        ...
        const imagePart = { inlineData: { data: Buffer.from(res.data, 'binary').toString("base64"), mimeType } };
        
        const visionRequest = ["Compose a Shakespearean Sonnet for this image.", imagePart];
        // Vision remains with Gemini
        const result = await generateWithFallback(VISION_MODEL_NAMES, visionRequest);

        const sonnet = result.response?.text?.();
        if (!sonnet) return `<!-- Vision skip: No content. -->\n`;
...
async function performYouTubeAnalysis(audioPart, youtubeUrl) {
    console.log(`Requesting YouTube analysis for ${youtubeUrl}...`);
    try {
        // Vision/Audio analysis also remains with Gemini
        const generationRequest = [YOUTUBE_ANALYSIS_PROMPT_UNIFIED, audioPart];
        const result = await generateWithFallback(TEXT_MODEL_NAMES, generationRequest);

        const analysisText = result.response.text();
        const markdown = `\n<details><summary>YouTube Audio Analysis</summary><pre><code>${escapeHtml(analysisText)}</code></pre></details>\n`;
        return { success: true, markdown: markdown, rawText: analysisText };
    } catch (error) {
        console.error(`Error during YouTube analysis for ${youtubeUrl}:`, error);
...
async function analyzeYouTubeAudioLocally(youtubeUrl) {
    ...
    try {
        ...
        if (createSnippet) {
            console.log("Command line switch '--create-snippet' detected. Creating audio snippet...");
            ...
            console.log("Skipping snippet creation as '--create-snippet' flag was not provided.");
        }
...
async function executeJamifyScript({ tags, timedLyricsJson, duration, jamifyConfig }) {
    ...
    const runJamify = async () => {
        try {
            ...
            const finalConfig = { ...baseConfig, ...jamifyConfig };
            ...
            if (!generatedFile) {
                 generatedFile = outputFiles.find(f => f.startsWith(id));
            }
            ...
            try { await fs.unlink(tempConfigPath); } catch (e) { /* ignore */ }
        }
    };
...
async function processSingleFile(inputFile, selectedPrompt) {
    ...
    try {
        ...
        fullTextContent = removeBoilerplate(fullTextContent.trim());
        
        if (!fullTextContent) { console.warn(`Skipping ${inputFile}: No text content after cleaning.`); return; }
        ...
        const textApiPromises = textChunks.map((chunk) => {
            ...
            if (useGrok) {
                return generateWithGrok(selectedPrompt.system, userPrompt);
            } else {
                const fullApiPrompt = `${selectedPrompt.system}\n\n${userPrompt}`;
                const generationRequest = { contents: [{ role: "user", parts: [{ text: fullApiPrompt }] }], generationConfig: { maxOutputTokens: 8192 } };
                return generateWithFallback(TEXT_MODEL_NAMES, generationRequest).catch(err => ({ error: true, message: err.message }));
            }
        });
...
<details><summary>Models & Prompt</summary><p><strong>Text:</strong> ${textModelInUse}</p><p><strong>Vision:</strong> ${VISION_MODEL_NAMES.join(' -> ')}</p><p><strong>Prompt (${selectedPrompt.name}):</strong></p><pre><code>${escapeHtml(selectedPrompt.system)}</code></pre><pre><code>${escapeHtml(selectedPrompt.chat)}</code></pre></details>`;
