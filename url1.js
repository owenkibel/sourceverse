    ...
    if (rawHtml) { // Proceed only if HTML was fetched
      try {
        ...
        $('body img').each((index, element) => {
          ...
          const alt = $(element).attr('alt'); // Optionally get alt text

          if (src) {
            try {
                // Resolve relative URLs to absolute URLs using the page's URL as the base
                const absoluteUrl = new URL(src, item.url).href;
                 imageSources.push({ // Store as object with src and optional alt
                    src: absoluteUrl,
                    alt: alt || null // Store alt text or null if not present
                 });
            } catch (urlError) {
                ...
                imageSources.push({ src: src, alt: alt || null });
            }
          }
        });
