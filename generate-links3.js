...
// 5000 = best quality/size balance. Override any time with: --transcript=8000
      ...
      const data = await page.evaluate(() => {
        ...
        if (images.length === 0) {
          images = Array.from(document.querySelectorAll('img[src^="http"]'))
            .map(el => el.src)
            .filter(src => src && src.match(/\.(jpg|jpeg|png|gif|webp)$/i) && !src.includes('profile') && !src.includes('avatar') && !src.includes('logo') && !src.includes('1x1'));
        }
...
async function runVersificationMode(url) {
  ...
  if (url.includes('youtube.com') || url.includes('youtu.be')) {
    const transcriptResult = await extractYouTubeTranscript(url, true);
    fullText = transcriptResult.transcript;
    try {
      const { result } = await ogs({ url, timeout: 12000 });
      if (result?.ogTitle) title = result.ogTitle;
    } catch (e) {}
  } else {
    let ogsSuccess = false;
    try {
      const { result, html } = await ogs({ url, timeout: 12000 });
      if (result?.ogTitle) title = result.ogTitle;
      if (html) {
    ...
    if (!ogsSuccess) {
      const pwData = await fetchWithPlaywright(url, sharedBrowser, true);
      if (pwData) {
  ...
  // OPTIMIZATION: Bump chunk boundaries to 20,000 chars for wider structural context
  ...
  for (let i = 0; i < chunks.length; i++) {
    console.log(`\n[Chunk ${i + 1}/${chunks.length}] Dispatching Context Block to Grok...`);
    
    let prompt = `You are Grok, an elite traditional poet. Adapt this text segment into high-quality, deeply insightful, truthful, and humorous traditional rhymed and metrical verse.
Guidelines:
...
${chunks[i]}`;
    ...
    try {
      const response = await fetch('https://api.x.ai/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.85,
          max_tokens: 4096, // Expanded window for robust outputs
        }),
      });

      if (!response.ok) throw new Error(`API Status: ${response.status}`);
      const apiData = await response.json();
      const stanzas = apiData.choices[0].message.content.trim();
  ...
  // --- ASTRO COMPATIBLE FRONT MATTER & MARKDOWN RESOLUTION ---
  const now = new Date();
...
async function processGroupToFolder(groupContent, folderName, enrichedData) {
  ...
  let poem = poemLines.join('\n').trim().replace(/\*{1,3}([^*]+)\*{1,3}/g, '$1').replace(/_{1,3}([^_]+)_{1,3}/g, '$1').replace(/[ \t]+$/gm, '').trim();
...
async function main() {
  try {
    ...
    if (urlArg) {
      await runVersificationMode(urlArg);
      return;
    }
    ...
    const enriched = await Promise.all(withDates.map(async (bm, index) => {
      ...
      try {
        return await timeout((async () => {
          ...
          } catch (e) { console.warn(`  OGS failed for ${bm.url}: ${e.message}`); }

          let transcriptResult = { transcript: '', cacheReused: false };
          if (bm.url.includes('youtube.com') || bm.url.includes('youtu.be')) {
            transcriptResult = await extractYouTubeTranscript(bm.url, false);
          }

          let descriptionParts = [];
          if (transcriptResult.transcript) descriptionParts.push(`YouTube Auto-Generated Transcript:\n\n${transcriptResult.transcript}`);
          ...
          if (!transcriptResult.transcript && !ogsSuccess) {
            ...
            const pwData = await fetchWithPlaywright(bm.url, sharedBrowser, false);
            if (pwData) {
...
Output EXACTLY the complete clean markdown body for the blog post (starting with the first ## header; no extra text, no introductions, no closing remarks, no separators).`;
    ...
    const response = await fetch('https://api.x.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: MODEL, messages: [{ role: 'user', content: fullPrompt }], temperature: 1, max_tokens: 8192 })
    });
