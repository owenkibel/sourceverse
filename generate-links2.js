...
function findBestMatchForUrl(url, enrichedData) {
  ...
  // 1. HIGHEST PRIORITY: Match YouTube videos strictly by immutable Video ID
  if (videoID) {
  ...
  // 2. Standard exact normalized URL match for articles and web links
  let match = enrichedData.find(item => normalizeUrl(item.url) === normalized);
  if (match) return match;

  // 3. Absolute last resort ONLY for non-YouTube links
  if (!videoID) {
...
function getYouTubeVideoID(url) {
  if (!url) return null;
  const regex = /(?:(?:[a-z0-9-]+\.)?youtube(?:-nocookie)?\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([^&\n?#]+)/i;
...
function normalizeUrl(u) {
  try {
    const parsed = new URL(u);
    // Secure query parameters for video watch indices
...
const timeout = (promise, ms, url) => {
  ...
  const timeoutPromise = new Promise((_, reject) => {
    timerId = setTimeout(() => reject(new Error(`Enrichment timeout after ${ms}ms for ${url}`)), ms);
  });
  
  return Promise.race([
    promise.then((value) => {
      clearTimeout(timerId); // Kill the pending timer instantly on success
      return value;
    }),
    timeoutPromise
  ]);
...
const response = await fetch('https://api.x.ai/v1/chat/completions', {
  ...
  body: JSON.stringify({
    ...
    temperature: 1,
    max_tokens: 8192, // Explicitly forces the API to unlock a generous output budget
    providerOptions: {
      xai: {
...
for (let i = 0; i < validGroups.length; i++) {
      ...
      // Explicitly terminate to spin down lingering sockets or handles instantly
      process.exit(0);
