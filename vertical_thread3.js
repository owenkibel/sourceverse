...
const useLens = args.includes('--lens'); // <-- ADD THIS LINE
...
// Load dramatic prompts (parallel to prompts-new)
async function loadDramaticPrompts() {
  try {
    const files = await fs.readdir(DRAMATIC_PROMPTS_DIR);
    const dramaticPrompts = {};
    for (const file of files) {
      if (!file.endsWith('.json')) continue;
      const content = await fs.readFile(path.join(DRAMATIC_PROMPTS_DIR, file), 'utf8');
      const prompt = JSON.parse(content);
      const key = file.replace('.json', '').toLowerCase().replace(/-/g, '_');
      dramaticPrompts[key] = prompt;
    }
    console.log(`✅ Loaded ${Object.keys(dramaticPrompts).length} dramatic prompt(s)`);
    return dramaticPrompts;
  } catch (e) {
    console.warn('   ⚠️ Could not load dramatic-prompts (OK on first run):', e.message);
    return {};
  }
}
...
// More forgiving Dramatic Verse extractor
function extractDramaticVerse(text) {
  // Try multiple possible patterns
  const patterns = [
    /##\s*DRAMATIC\s*VERSE\s*\(Act\s*(\d+)\)\s*\n([\s\S]*?)(?=\n##|$)/i,
    /##\s*Dramatic Verse.*?(Act\s*\d+)/i,
    /\*\*Dramatic Verse.*?(Act\s*\d+)\*\*/i,
    /DRAMATIC VERSE.*?Act\s*(\d+)/i
  ];
  ...
  for (const regex of patterns) {
    ...
    if (match) {
      const act = parseInt(match[1]) || 1;
      let content = match[2] ? match[2].trim() : text.substring(text.indexOf(match[0]) + match[0].length).trim();
      return { act, content: content || match[0] };
    }
  }
  return null;
}
...
async function runImageGen(prompt) {
  ...
  try {
    ...
    if (refineWithOmniGen) {
        ...
        // <-- ADD THIS BLOCK
        console.log(`Triggering direct Text-to-Image via Lens-Turbo...`);
        await execFileAsync('bun', ['run_lens.js', '--state-file', stateFile]);
    } else {
    ...
    else if (useLens) engineName = "Lens-Turbo"; // <-- ADD THIS LINE
...
async function main() {
  ...
  const dramaticPrompts = await loadDramaticPrompts();
...
if (modelUpdateContent) {
  ...
  console.log("   🔍 First 1500 chars of generated output:\n", generated.substring(0, 1500));
     ...
     // === DOMAIN-AWARE DRAMATIC VERSE GENERATION ===
    let dramaticResult = null;
    if (Object.keys(dramaticPrompts).length > 0) {
      console.log(`   🎭 Generating Dramatic Verse (Act continuation)...`);

      // === DOMAIN DETECTION & PROMPT SELECTION ===
      let chosenKey = 'dramatic_verse_base';   // default fallback

      const domainText = (domainHint + " " + richContextBlock).toLowerCase();

      if (domainText.includes("political")) {
        chosenKey = 'political_dramatic_voice';
        console.log(`   🎭 → Using POLITICAL dramatic voice`);
      } else {
        chosenKey = 'dramatic_verse_base';
        console.log(`   🎭 → Using BASE dramatic voice (non-political domain)`);
      }

      const dramPrompt = dramaticPrompts[chosenKey];

      // Build prompt with domain awareness
      let dramaticUserPrompt = dramPrompt.chat.replace('[[chunk]]', richContextBlock || '');
      dramaticUserPrompt += modelContext + `\n\nDOMAIN FOR THIS CYCLE: ${domainHint}\n\nCRITICAL: You MUST output the dramatic section using EXACTLY this header on its own line:\n## DRAMATIC VERSE (Act {{nextActNumber}})\nThen write the scene in verse with characters and chorus. Do not deviate from this format.`;

      const dramaticGenerated = await generateText(dramPrompt.system, dramaticUserPrompt);
      
      dramaticResult = extractDramaticVerse(dramaticGenerated);
      if (dramaticResult) {
        console.log(`   ✅ Extracted Dramatic Verse Act ${dramaticResult.act} using ${chosenKey}`);
      } else {
        console.warn(`   ⚠️ Could not extract Dramatic Verse section`);
      }
    }
...
${dramaticResult ? `
## DRAMATIC VERSE (Act ${dramaticResult.act}) {#dramatic-verse}
${dramaticResult.content}
` : ''}
