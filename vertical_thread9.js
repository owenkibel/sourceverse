...
async function generateText(system, user) {
  ...
  } // Inside your text generation engine block
else {
  console.log("Generating with Gemini...");
  for (const modelName of ["gemini-3.1-pro-preview", "gemini-3-flash-preview"]) {
    try {
      // Pass system rules into the native config parameter
      const model = genAI.getGenerativeModel({ 
        model: modelName, 
        generationConfig: { temperature: 1 },
        systemInstruction: system // Enforces ironclad guardrails
      });
      
      // Send ONLY the user data payload to the generation call
...
// Change your function definition inside vertical_thread9.js to strip out the echo chamber
async function buildNarrativeContext(domain, cumulativeModel) {
    if (!cumulativeModel.narrativeArcs || !cumulativeModel.narrativeArcs[domain]) {
        return { context: "Beginning of the narrative lineage for this node.", injected: 0 };
    }

    const arc = cumulativeModel.narrativeArcs[domain];
    
    // CLEANSE THE CONTEXT: Instruct the model that anything inside brackets is historical legacy data
    let context = `--- HISTORICAL RECORD ARCHIVE ---\n`;
    context += `${arc.currentArc || ''}\n\n`;
    context += `CRITICAL RUNTIME INSTRUCTION: Treat the [Recent Forecast Summary] entries above purely as historical benchmarks. Do not reuse their metaphors, descriptions, or names for your new output sections.\n\n`;

    let injectedCount = 0;

    // Inject recent forecasts strictly under a dedicated telemetry header
    if (arc.forecastHistory && arc.forecastHistory.length > 0) {
        const recentForecasts = arc.forecastHistory.slice(-2);

        context += "--- RECENT PREDICTIVE TELEMETRY (For Continuity Balance) ---\n";
        recentForecasts.forEach((f, i) => {
            const shortForecast = f.forecast.length > 220 
                ? f.forecast.substring(0, 217) + "..." 
                : f.forecast;
            context += `Act ${f.act} Target Benchmark: ${shortForecast}\n`;
        });

        injectedCount = recentForecasts.length;
        console.log(`   📡 Context Cleaned: Injected ${injectedCount} historical benchmarks into stream wrapper.`);
    }
