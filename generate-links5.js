...
async function extractAndAnalyzeAudio(url) {
  ...
  try {
    ...
    // Fallback format matrix handles audio tracks across standard videos and specialized music codecs cleanly
    await execAsync(
...
${chunks[i]}`;
  ...
  // Inject line breaks into the individual standalone verse document text
  let inStandalonePoem = true;
...
async function processGroupToFolder(groupContent, folderName, enrichedData) {
  ...
  for (let i = 1; i < lines.length; i++) {
    const line = lines[i];
    // FIXED: Retain empty structural entries to preserve stanza spacing definitions
    if (!line.trim()) {
      if (!foundFirstBullet) {
        poemLines.push(''); 
      }
  ...
  // FIXED: Format text fragments to inject the Markdown double-space newline marker sequence
    ...
    const enriched = await Promise.all(withDates.map(async (bm, index) => {
      ...
      try {
        return await timeout((async () => {
          // FIXED: Map target URLs to reference standard YouTube interfaces for media processing dependencies
...
Output EXACTLY the complete clean markdown body for the blog post (starting with the first ## header; no extra text, no introductions, no closing remarks, no separators).`;
    ...
    // FIXED: Global string processor scans the master document structure and injects trailing spaces into the poem boundaries
