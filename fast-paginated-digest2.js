...
const MAX_PAGE_SIZE = 50; // Configurable upper limit target per post
...
/**
 * Extracts YouTube Video ID if present in the URL
 */
...
/**
 * Transforms music.youtube.com into www.youtube.com specifically for scraping
 */
function getScrapeUrl(urlStr) {
  try {
    const parsed = new URL(urlStr);
    if (parsed.hostname.toLowerCase() === 'music.youtube.com') {
...
function parseMetadata(html) {
  ...
  // 1. Title Lookup Matrix (Prioritize og:title / twitter:title over fallback <title>)
  const ogTitleMatch = html.match(/<meta[^>]+(?:name|property)=["'](?:og:title|twitter:title)["'][^>]+content=["']([^"']*)["']/i) ||
                       html.match(/<meta[^>]+content=["']([^"']*)["'][^>]+(?:name|property)=["'](?:og:title|twitter:title)["']/i);
  if (ogTitleMatch && ogTitleMatch[1]) {
    ...
    if (titleMatch && titleMatch[1]) {
      ...
      if (!rawTitle.toLowerCase().includes('your browser is deprecated')) {
        title = rawTitle;
      }
    }
  }

  // 2. Description Lookup Matrix
  ...
  if (descMatch && descMatch[1]) description = decodeHtmlEntities(descMatch[1].trim());

  // 3. Image Asset Allocation
  const imgMatch = html.match(/<meta[^>]+(?:name|property)=["'](?:og:image|twitter:image)["'][^>]+content=["']([^"']*)["']/i) ||
                   html.match(/<meta[^>]+content=["']([^"']*)["'][^>]+(?:name|property)=["'](?:og:image|twitter:image)["']/i);
  if (imgMatch && imgMatch[1]) image = decodeHtmlEntities(imgMatch[1].trim());

  // 4. Article Publication Dateline Target Selection
  ...
  if (dateMatch && dateMatch[1]) publishedTime = decodeHtmlEntities(dateMatch[1].trim());

  // 5. Source Platform Extraction (og:site_name)
  const siteMatch = html.match(/<meta[^>]+(?:name|property)=["']og:site_name["'][^>]+content=["']([^"']*)["']/i) ||
                    html.match(/<meta[^>]+content=["']([^"']*)["'][^>]+(?:name|property)=["']og:site_name["']/i);
  if (siteMatch && siteMatch[1]) siteName = decodeHtmlEntities(siteMatch[1].trim());

  // 6. Author/Creator Attribution Tracking
  ...
  if (authorMatch && authorMatch[1]) author = decodeHtmlEntities(authorMatch[1].trim());

  return { title, description, image, publishedTime, siteName, author };
}

async function fastScrape(url) {
  const scrapeTarget = getScrapeUrl(url);
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);

  try {
    const response = await fetch(scrapeTarget, {
...
async function main() {
  ...
  if (isFirstRun) {
    console.log(`🚀 First Run Baseline Anchor: Sampling latest ${MAX_PAGE_SIZE} elements.`);
    toProcess = bookmarksPool.slice(0, MAX_PAGE_SIZE);
  } else {
  ...
  for (const entry of toProcess) {
    ...
    const isMusicUrl = entry.url.includes('music.youtube.com');
    const isYouTubeUrl = isMusicUrl || entry.url.includes('youtube.com') || entry.url.includes('youtu.be');

    // 1. Perform HTTP Scrape
    if (checkProblematicDomain(entry.url)) {
      console.log(`🛑 Blocked Domain: Local metadata match preserved for ${entry.url}`);
    } else {
    ...
    // 2. Promote YouTube / YouTube Music thumbnail resolutions if mqdefault was returned
    if (image && image.includes('ytimg.com')) {
      image = image.replace('/mqdefault.', '/hqdefault.');
    }

    // 3. Fallback: Generate video thumbnail if no image was scraped
    if (!image && isYouTubeUrl) {
    ...
    // 4. Enforce accurate platform name
    if (isMusicUrl) {
      siteName = 'YouTube Music';
    } else if (!siteName && isYouTubeUrl) {
      siteName = 'YouTube';
    }
  ...
  const chunks = [];
  const totalItems = scrapedItems.length;

  if (totalItems <= MAX_PAGE_SIZE) {
    chunks.push(scrapedItems);
  } else {
    ...
    while (remainingPages > 0) {
      const currentChunkSize = Math.ceil(remainingItems / remainingPages);
      chunks.push(scrapedItems.slice(cursor, cursor + currentChunkSize));
      cursor += currentChunkSize;
      remainingItems -= currentChunkSize;
      remainingPages--;
    }
  }
