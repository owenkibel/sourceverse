...
if (ignoreIdx !== -1) {
  for (let i = ignoreIdx + 1; i < process.argv.length; i++) {
    const arg = process.argv[i];
    if (arg.startsWith('--')) break;
    ignoreArgs.push(arg.replace(/\/$/, '')); // normalize trailing slash
  }
}

function shouldIgnoreUrl(url) {
  if (!ignoreArgs.length) return false;
  const normalized = url.toLowerCase();
  return ignoreArgs.some(prefix => {
    const p = prefix.toLowerCase();
    // Match hostname or full path prefix
