...
async function main() {
  ...
  // --- NEW BALANCED CHUNKING LOGIC ---
  const chunks = [];
  const totalItems = scrapedItems.length;

  if (totalItems <= MAX_PAGE_SIZE) {
    // If within upper limits, throw them all into a single page
    chunks.push(scrapedItems);
  } else {
    // Distribute remaining URLs dynamically to eliminate small trailing files
