 ...
 * course_builder4.js
 *
 * Same isolation rules as v3. Adds Humor and Satire as separate craft
 * series (one mode per run — do not mix them in one --title).
 *
 * Modes:
 *   --mode=course   analyst prompt  (Demagoguery 101)
 *   --mode=prompt   craft prompt    (Snark 101; aliases: snark, craft)
 *   --mode=humor    humor prompt    (Humor 101)
 *   --mode=satire   satire prompt   (Satire 101)
 *
 * Writes:
 *   cumulative_course_model.json
 *   courses/<slug>/index.md
 *   courses/<slug>/run-NNN.md
 *   courses/<slug>/_prompt.md
 *   courses/<slug>/prompt.md           only with --publish-prompt
 *
 * Usage:
 *   bun course_builder4.js --url="https://..." --title="Demagoguery 101"
 *   bun course_builder4.js --url="https://futurism.com/..." --title="Snark 101" --mode=prompt
 *   bun course_builder4.js --url="https://..." --title="Humor 101" --mode=humor
 *   bun course_builder4.js --url="https://..." --title="Satire 101" --mode=satire
 */
...
function promptRulesFor(mode, spec) {
  ...
  if (mode === 'prompt') {
    return `Write a CRAFT system prompt in the second person. It teaches a model to recognize and, if asked, reconstruct the desk's moves. ${cap}`;
  }
  if (mode === 'humor') {
...
async function main() {
  if (!urlArg) {
    ...
    console.error('  bun course_builder4.js --url="https://..." --title="Demagoguery 101"');
    console.error('  bun course_builder4.js --url="https://..." --title="Snark 101" --mode=prompt');
    console.error('  bun course_builder4.js --url="https://..." --title="Humor 101" --mode=humor');
    console.error('  bun course_builder4.js --url="https://..." --title="Satire 101" --mode=satire');
    process.exit(1);
  }
...
main().catch((err) => {
  console.error('Fatal course_builder4 error:', err);
  process.exit(1);
});
