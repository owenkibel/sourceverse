...
let textModel, visionModel, transcriptionModel;
let googleGenAIClient;
let logMessage;

try {
    textModel = genAI.getGenerativeModel({ model: TEXT_MODEL_NAME, safetySettings, generationConfig: { temperature: 1.0 } });
    transcriptionModel = genAI.getGenerativeModel({ model: TEXT_MODEL_NAME, safetySettings, generationConfig: { temperature: 0.2 } });
    ...
    console.log(`- Creative Text (${TEXT_MODEL_NAME}, temp: 1.0)`);
    console.log(`- Factual Text/Transcription (${TEXT_MODEL_NAME}, temp: 0.2)`);
    console.log(`- Vision (${VISION_MODEL_NAME})`);
...
async function generateAndEmbedVideoVeo2(videoPromptContent, baseFilename) {
    ...
    try {
        let operation = await googleGenAIClient.models.generateVideos({
            ...
            config: {
                numberOfVideos: 1, // Reduced to 1 to manage potential quota issues on fallback
                durationSeconds: 12, // Increased duration
...
Analyze the provided video's audio track to produce a clean and accurate transcript.
- Identify individual speakers and label them clearly (e.g., "Male Speaker:", "Female Speaker:").
- If a known public figure is speaking, use their name as the label (e.g., "Elon Musk:").
- Transcribe all spoken words, including conversations, speeches, and song lyrics, as accurately as possible.
- Do not add any commentary, analysis, or description of sounds. The output should only be the formatted transcript.
`;

const YOUTUBE_ANALYSIS_PROMPT = `
Based on the provided audio and its transcript, create a detailed analysis of the video's soundscape and a prompt for music generation.

### Audio Analysis
Describe the non-speech audio elements in a detailed and interesting way. Pay attention to:
- **Soundscape:** Natural sounds, ambient noise (e.g., city traffic, forest sounds), sound effects.
- **Music:** Describe the music in a detailed, interesting, and historical way. Analyze genre, instrumentation, composition techniques, and emotional tone.
- **Voice Quality:** Describe the emotional tone, accent, and vocal characteristics of the speakers.

### Music Generation Prompt
Generate a highly detailed, plain text, comma-delimited, single-line text prompt suitable for an AI music generation model, approximately 150 words in length. This prompt should enable the creation of new music in the genre and spirit of the music analyzed.
- **Structure:** Begin with generic terms (era, genre, mood) and progressively add specific details.
- **Content:** Include a rich description of the musical era, genre, instrumentation (from broad families to specific instruments), vocal characteristics (soloists, choirs, style), tempo, dynamics, and overall theme.
- **Constraint:** If a specific composer or artist is identifiable, their name must ONLY be mentioned at the very end of the prompt.
...
async function performYouTubeAnalysis(audioPart, youtubeUrl, startTime = 'N/A') {
    ...
    console.log(`Requesting accurate transcript for ${youtubeUrl}...`);
    try {
        const transcriptResult = await transcriptionModel.generateContent([YOUTUBE_TRANSCRIPT_PROMPT, audioPart]);
        transcriptText = transcriptResult.response.text();
        combinedMarkdown += `\n### YouTube Transcript (from ${startTime})\n<pre><code>${escapeHtml(transcriptText)}</code></pre>\n`;
    } catch (error) {
        console.error(`Error during YouTube transcription for ${youtubeUrl}:`, error);
        errors.push(`Transcription failed: ${error.message}`);
        success = false;
    }

    // Part 2: Detailed Sound Analysis
    if (success) { // Only proceed if transcription was successful
        console.log(`Requesting detailed audio analysis for ${youtubeUrl}...`);
        try {
            // Pass both the audio and the new transcript for full context
            const analysisResult = await textModel.generateContent([YOUTUBE_ANALYSIS_PROMPT, {text: transcriptText}, audioPart]);
            analysisText = analysisResult.response.text();
            combinedMarkdown += `\n### YouTube Audio Analysis\n<pre><code>${escapeHtml(analysisText)}</code></pre>\n`;
        } catch (error) {
            console.error(`Error during YouTube sound analysis for ${youtubeUrl}:`, error);
            errors.push(`Sound analysis failed: ${error.message}`);
            // This part failing is not critical enough to mark the whole process as a failure
        }
    }
    ...
    return { success: true, markdown: combinedMarkdown };
}
...
async function analyzeYouTubeAudioLocally(youtubeUrl, startTime = '0') {
    ...
    try {
        ...
        const args = [ '-x', '--audio-format', 'opus', '--ppa', `ffmpeg:${ffmpegArgs}`, '-o', tempFilePath, youtubeUrl ];
        console.log(`Executing command: yt-dlp ${args.join(' ')}`);
        const { stdout, stderr } = await execFileAsync('yt-dlp', args);
        if (stderr) console.warn('yt-dlp stderr output (this is usually download progress):\n', stderr);
        ...
        return await performYouTubeAnalysis(audioPart, youtubeUrl, startTime);
...
<p><strong>Text (Creative):</strong> ${TEXT_MODEL_NAME} (temp: 1.0)<br><strong>Text (Factual):</strong> ${TEXT_MODEL_NAME} (temp: 0.2)<br><strong>Vision:</strong> ${VISION_MODEL_NAME}<br><strong>Image Gen:</strong> ${IMAGE_GEN_MODEL_NAME}<br><strong>TTS:</strong> ${TTS_MODEL_DISPLAY_NAME}<br><strong>Video:</strong> ${videoModelUsed}</p>
