...
async function analyzeYouTubeAudioLocally(youtubeUrl) {
    ...
    try {
        const args = ['-f', 'bestaudio/best', '-x', '--audio-format', 'opus', '-o', tempFilePath, youtubeUrl];
        ...
        // Side-effect: Create a 30-second MP3 snippet for Jamify
        ...
        try {
            ...
            console.log(`Successfully created 30-second snippet: ${snippetPath}`);
        } catch (ffmpegError) {
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                ffmpeg(generatedFilePath)
                    ...
                    .toFormat('opus')
                    .on('end', () => {
                        fs.unlink(generatedFilePath).catch(() => {});
                        resolve();
                    })
                    .on('error', (err) => {
