...
const useLimericks = args.includes('--limericks'); // <-- Add this switch
...
// Build hypothesis block
    ...
    if (useLimericks) {
      try {
        ...
        // Executes synchronously against your Fish environment environment binaries
        const systemLimerick = execSync('fortune limericks', { encoding: 'utf8' }).trim();
        
        hypothesisBlock = `### RUNTIME HYPOTHESES IN PLAY (SUBVERTED VIA SYSTEM LIMERICK):\n1. [THEORY] ${systemLimerick}\n`;
      } catch (err) {
        console.warn("   ⚠️ Scriptorium Alert: fortune utility execution dropped, falling back down to ledger lines:", err.message);
        useLimericksFallback = true;
      }
    }

    // Fallback block if flag is absent or system binary execution fails
    if (!useLimericks || !hypothesisBlock) {
...
{"aspect_ratio":"9:16","high_level_description":"A theatrical stage set layout...","compositional_deconstruction":{"background":"A stark digital lattice shell...","elements":[{"type":"obj","bbox":[450,200,850,750],"desc":"A glowing artifact center stage"}]}}`;
    ...
    // 5. STANDARD TOKEN GLOBAL REPLACEMENTS[cite: 11]
    userPrompt = userPrompt.replace(/\[\[narrative_context\]\]/g, narrativeContext); //[cite: 11]
    ...
    userPrompt = userPrompt.replace(/\[\[chunk\]\]/g, richContextBlock); //[cite: 11]

    // High-uniformity Fisher-Yates shuffle array placement
    const dynamicStylesString = getShuffledAceStyles(); //[cite: 11]
    userPrompt = userPrompt.replace(/\[\[ace_styles\]\]/g, dynamicStylesString); //[cite: 11]

    userPrompt = userPrompt.replace(/\[\[act_number\]\]/g, nextActNumber.toString()); //[cite: 11]

    // Inject hypotheses blocks[cite: 11]
    if (userPrompt.includes('[[hypotheses_block]]')) {
      userPrompt = userPrompt.replace(/\[\[hypotheses_block\]\]/g, hypothesisBlock); //[cite: 11]
    } else {
      userPrompt += `\n\n${hypothesisBlock}`; //[cite: 11]
    }
