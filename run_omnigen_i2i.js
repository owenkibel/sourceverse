...
const POSITIVE_PROMPT_NODE = "6"; 
const LOAD_IMAGE_NODE = "16";
const SEED_NODE = "21";        
...
async function uploadImageToComfy(imagePath) {
    ...
    console.log(`Uploading anchor image to ComfyUI: ${imagePath}...`);
    
    // Read file and format it for web upload
    ...
    if (!res.ok) throw new Error(`Failed to upload image: ${await res.text()}`);
    
    const data = await res.json();
    return data.name; // ComfyUI returns the internal filename it assigned
}
...
async function main() {
    try {
        ...
        const inputImageIndex = args.findIndex(a => a === '--input-image');
        const inputImagePath = inputImageIndex !== -1 ? args[inputImageIndex + 1] : './anchor.png';

        // 2. Read the prompt
        let imagePrompt = "Transform into a highly detailed cybernetic sci-fi masterpiece";
        if (fs.existsSync('prompt.txt')) {
            imagePrompt = fs.readFileSync('prompt.txt', 'utf8').trim();
        }

        console.log(`\n🌌 Mode: OmniGen2 Image-to-Image`);
        console.log(`Injecting Prompt: ${imagePrompt.substring(0, 50)}...`);

        // 3. Upload the anchor image and get the internal filename
        const comfyFileName = await uploadImageToComfy(inputImagePath);

        // 4. Load the JSON Workflow
        const workflowPath = path.join(process.cwd(), 'image_omnigen2_image_edit_gguf.json');
        ...
        // 5. Inject the Prompt, Seed, and Image Filename
        const seed = Math.floor(Math.random() * 100000000000000);
        
        promptData[POSITIVE_PROMPT_NODE].inputs.text = imagePrompt;  
        promptData[SEED_NODE].inputs.noise_seed = seed;         
        promptData[LOAD_IMAGE_NODE].inputs.image = comfyFileName;
