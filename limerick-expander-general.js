...
- Wrap ONLY the 5-line poem inside <limerick>...</limerick> tags.`;
  ...
  // Universal payload configuration
  const payload = {
    temperature: 0.7,
    top_p: 0.9,
    min_p: 0.05,
  ...
  if (isGemma) {
    // For Gemma: Combine into user role and disable thinking loops
    payload.messages = [
      { role: 'user', content: `${systemInstructions}\n\n${userInstructions}` }
    ];
    payload.chat_template_kwargs = { enable_thinking: false };
  } else {
    // For Hermes & Grok-Apollyon: Standard ChatML system + user turns
    payload.messages = [
...
async function main() {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    try {
      // Pass modelName into the function:
