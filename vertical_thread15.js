...
// Generalized matcher catches any genre combination ending in vocal tags
userPrompt = userPrompt.replace(
  /TAGS:\s*.*?(Bass-Baritone|Lyric Tenor|Mezzo-Soprano|Dramatic Soprano)\s*(male|female)?\s*vocals/gi,
  `TAGS: Folk, Americana, Instrumental, ${randomVoiceBaseline}`
);
...
// =========================================================================
// VOCAL-PRESERVING MUSIC TAG SANITIZATION & GUARANTEED BALANCE
// =========================================================================
let rawTagsString = (parsed.musicTags || "").replace(/[\[\]()]/g, ''); 

// Extract vocal tracking metrics
...
// GUARANTEED FALLBACK: If LLM omitted vocal profile, enforce randomVoiceBaseline
if (!preservedVoiceTag) {
    preservedVoiceTag = randomVoiceBaseline;
    console.log(`🎲 LLM omitted vocal tag. Programmatically enforcing balanced voice: "${preservedVoiceTag}"`);
}
