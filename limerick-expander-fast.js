...
function cleanLimerick(rawText) {
  // 1. Strip reasoning scratchpads (both closed and unclosed)
  let clean = rawText
    .replace(/<think>[\s\S]*?<\/think>/gi, '')
    .replace(/<think>[\s\S]*/gi, '')
    .trim();

  // 2. Primary: Extract content strictly between <limerick> tags
  const tagMatch = clean.match(/<limerick>([\s\S]*?)<\/limerick>/i);
  if (tagMatch) {
    return tagMatch[1].trim();
  }

  // 3. Fallback: Strip stray formatting or markdown tags
  clean = clean.replace(/<\/?(limerick|think)>/gi, '').trim();
...
async function expandSeedWithLLM(seedPhrase, modelName = "") {
  ...
  // Strict micro-thinking instruction: limit planning to just picking rhymes
  const systemInstructions = `You are a master of comic satire and witty botanical verse.
You specialize in classic 5-line limericks with flawless AABBA meter and sharp botanical double-entendres.
Keep internal <think> reasoning under 30 words: quickly select 3 A-rhymes and 2 B-rhymes, then close </think> immediately.
Always output the 5-line poem wrapped strictly inside <limerick>...</limerick> tags.`;
