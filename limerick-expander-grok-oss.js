...
async function getActiveModelName() {
  try {
    ...
    // Extract raw model path/id from llama-server
    const rawPath = data.data?.[0]?.id || "";
    
    // Return clean filename without path and .gguf extension
...
Never use slant rhymes or half-rhymes. Sexual evocativeness or innuendo is fine but do not use demeaning filthy words like synonyms for feces. Every rhyme must be clean and audible. `;

  const userPrompt = `Write a classic 5-line limerick based on this comical theme. The last line must be introduced to have an unexpected witty twist as a punchline.
"${seedPhrase}"
...
author: "Owen Kibel"
model: "${modelName}"
tags: ["limericks", "generative-poetry", "${modelName.toLowerCase()}", "botanical"]
---
