...
let textModel = genAI.getGenerativeModel({ model: TEXT_MODEL_NAME, safetySettings, generationConfig: { temperature: 1 } });
let visionModel = genAI.getGenerativeModel({ model: VISION_MODEL_NAME, safetySettings });
...
function extractLyricStanza(fullVerse) { 
    ...
    for (const stanza of potentialStanzas) { 
        ...
        // --- CHANGE THIS VALUE ---
        if (lineCount >= 8) break; // Increased the line limit for longer lyrics
    } 
...
function generateTimedLyricsFromJson(stanza) {
    ...
    const timedLyrics = [];
    const LYRICS_START_SECOND = 10.0;
    let currentTime = LYRICS_START_SECOND + 0.5; // Start lyrics at 10 seconds + 0.5s leading
    ...
    totalDuration = Math.min(totalDuration, 60.0); // Cap at 15 seconds
...
async function analyzeYouTubeAudioLocally(youtubeUrl) { console.log(`Performing local audio analysis for: ${youtubeUrl}.`); const tempFileName = `temp-audio-${Date.now()}.opus`; const tempFilePath = path.join(os.tmpdir(), tempFileName); try { const args = ['-f', 'bestaudio/best', '-x', '--audio-format', 'opus', '--ppa', 'ffmpeg:-t 600', '-o', tempFilePath, youtubeUrl]; if (fss.existsSync(YT_DLP_COOKIES_PATH)) { console.log("Found cookies.txt, using it for yt-dlp download."); args.unshift('--cookies', YT_DLP_COOKIES_PATH); } else { console.log("No cookies.txt found, attempting download without them."); } await execFileAsync('yt-dlp', args); const audioFileBuffer = await fs.readFile(tempFilePath); const audioPart = { inlineData: { data: audioFileBuffer.toString("base64"), mimeType: "audio/opus" } }; return await performYouTubeAnalysis(audioPart, youtubeUrl); } catch (error) { console.error(`Local YouTube audio analysis failed for ${youtubeUrl}:`, error); if (error.message.includes('HTTP Error 403')) { console.error("\n>>> HINT: A 403 Forbidden error often means YouTube is blocking requests. Ensure your 'cookies.txt' file is present and up-to-date.\n"); } return { success: false, error: error.message }; } finally { try { await fs.unlink(tempFilePath); } catch (e) { /* ignore */ } } }
...
async function executeJamifyScript({ tags, timedLyricsJson, duration }) {
    ...
    const runJamify = async () => {
        try {
            const audioFiles = await fs.readdir(INPUT_AUDIO_DIR_JAMIFY);
            const validAudio = audioFiles.filter(f => f.toLowerCase().endsWith('.wav') || f.toLowerCase().endsWith('.mp3'));
            if (validAudio.length === 0) throw new Error('No .wav or .mp3 files found in Jamify input directory.');
            const selectedAudioFile = validAudio[Math.floor(Math.random() * validAudio.length)];
            console.log(`Selected Jamify input audio: ${selectedAudioFile}`);
            ...
            const inputData = [
                {
                    "id": id,
                    "audio_path": `inputs/${selectedAudioFile}`,
                    "lrc_path": "inputs/timed_lyrics.json",
                    "duration": duration,
            ...
            let innerCommand = `python -c "import torch; torch.cuda.empty_cache()" && accelerate launch --mixed_precision=fp16 inference.py`;
            if (useCpu) {
                innerCommand = `accelerate launch --mixed_precision=fp16 --cpu inference.py`;
            }
            ...
            const generatedFile = outputFiles.find(f => f.startsWith(id));
            ...
            await new Promise((resolve, reject) => {
                ffmpeg(generatedFilePath)
                    ...
                    .toFormat('opus')
                    .on('end', () => {
                        fs.unlink(generatedFilePath).catch(() => {}); // Clean up original file from Jamify's output dir
                        resolve();
                    })
                    .on('error', (err) => {
                        reject(new Error(`FFmpeg conversion failed: ${err.message}`));
                    })
                    .save(absoluteFinalPath); // Save directly to the final destination
            });
            ...
            return { 
                success: false, 
                markdown: `<p><strong>Jamify failed.</strong> Error: ${escapeHtml(error.message)}</p>`, 
                prompts: { tags, timedLyricsJson, duration } 
            };
        }
    };
...
async function processSingleFile(inputFile, selectedPrompt) {
    ...
    try {
        ...
        if (youtubeAnalysisResult && youtubeAnalysisResult.success && youtubeAnalysisResult.rawText) {
            const match = youtubeAnalysisResult.rawText.match(/###\s*Music Tags:([\s\S]*?)(?=###|$)/i);
            if (match && match[1]) {
        ...
        if (allPrompts.music_tags.length === 0) {
            ...
            allPrompts.music_tags.push("cinematic, orchestral, inspirational, uplifting, powerful, instrumental, epic score");
        }
        ...
        let timedLyricsForJamify = null;
                let jamifyDuration = 60.0; // Default to 30 seconds max
        if (timedLyricsResult) {
            ({ timedLyrics: timedLyricsForJamify, totalDuration: jamifyDuration } = timedLyricsResult);
            // --- CHANGE THIS VALUE ---
            jamifyDuration = Math.min(jamifyDuration, 60.0); // Override to 30 seconds max
        }
        ...
        const musicGenResult = await generateMusicWithComfyUI({
            ...
            negative_tags: allPrompts.negative_tags[0]
        });
