 ...
 * Finds every <audio> tag and inserts a pitch-colored scrolling spectrogram.
...
(() => {
  ...
  if (window.__chromaDriftBooted) return;
...
.cd-stage{position:relative;height:260px;background:#050506;touch-action:none}
@media (max-width:640px){.cd-stage{height:208px}.cd-legend{display:none}}
.cd-stage canvas{display:block;width:100%;height:100%;cursor:ew-resize;mask-image:linear-gradient(90deg,transparent 0%,#000 8%,#000 100%)}
.cd-playhead{position:absolute;top:0;bottom:0;width:1px;left:28%;background:rgba(236,236,232,.88);box-shadow:0 0 14px 2px rgba(236,236,232,.28);pointer-events:none}
.cd-seam{position:absolute;left:0;right:0;top:50%;height:1px;background:rgba(236,236,232,.14);pointer-events:none}
.cd-lr{position:absolute;left:10px;font:600 10px/1 "IBM Plex Sans",system-ui,sans-serif;letter-spacing:.16em;text-transform:uppercase;pointer-events:none}
.cd-lr-l{top:10px;color:#3dffb0}
.cd-lr-r{bottom:10px;color:#3db0ff}
...
`;
  ...
  function writeNotePixel(p, w, x, y0, col, v) {
    if (v < 0.025) return;
    const hot = v * v * v;
    const r = Math.min(255, col[0] * v + 255 * hot);
    const g = Math.min(255, col[1] * v + 255 * hot);
    const b = Math.min(255, col[2] * v + 255 * hot);
    const a = Math.min(255, (v * 1.25 * 255) | 0);
    for (let dy = 0; dy < NOTE_PX; dy++) {
      const edge = dy === 0 || dy === NOTE_PX - 1 ? 0.5 : 1;
      const i = ((y0 + dy) * w + x) << 2;
      p[i] = r * edge;
      p[i + 1] = g * edge;
      p[i + 2] = b * edge;
  ...
  function renderSpec(lFrames, rFrames) {
    ...
    const h = N_NOTES * NOTE_PX * 2 + GAP_PX;
    const canvas = document.createElement("canvas");
    canvas.width = Math.max(1, nFrames);
    canvas.height = h;
    const ctx = canvas.getContext("2d", { alpha: true });
    const img = ctx.createImageData(nFrames, h);
    const p = img.data;
    const botBase = N_NOTES * NOTE_PX + GAP_PX;
    for (let x = 0; x < nFrames; x++) {
      const off = x * N_NOTES;
      for (let n = 0; n < N_NOTES; n++) {
        const col = CHROMA[(NOTE_MIN + n) % 12];
        writeNotePixel(p, nFrames, x, (N_NOTES - 1 - n) * NOTE_PX, col, lFrames[off + n]);
        writeNotePixel(p, nFrames, x, botBase + n * NOTE_PX, col, rFrames[off + n]);
      }
    }
  ...
  function drawOverlay(ctx, w, h) {
    ...
    for (const midi of labels) {
      ...
      ctx.fillText(name, 10, usable * (1 - t) + 1);
      ctx.fillText(name, 10, half + gap + usable * t);
    }
    ctx.restore();
  }

  function scrollAndDraw(g, w, h, playX, notesL, notesR) {
    try {
      ...
      const gap = 3;
      const usable = half - gap;
      const paintBand = (notes, mirrorHighAtTop, yBase, yScale) => {
        for (let n = 0; n < N_NOTES; n++) {
          const v = notes[n];
          if (v < 0.03) continue;
          const rgb = CHROMA[(NOTE_MIN + n) % 12];
          const hot = v * v * v;
          const r = Math.min(255, rgb[0] * v + 255 * hot) | 0;
          const gc = Math.min(255, rgb[1] * v + 255 * hot) | 0;
          const b = Math.min(255, rgb[2] * v + 255 * hot) | 0;
          const a = Math.min(255, (v * 255 * 1.2) | 0);
          const t0 = mirrorHighAtTop ? 1 - (n + 1) / N_NOTES : n / N_NOTES;
          const t1 = mirrorHighAtTop ? 1 - n / N_NOTES : (n + 1) / N_NOTES;
          const y0 = (yBase + t0 * yScale) | 0;
          const y1 = Math.max(y0 + 1, (yBase + t1 * yScale) | 0);
          const x = playX - 1;
      ...
      paintBand(notesL, true, 0, usable);
      paintBand(notesR, false, half + gap, usable);
  ...
  function enhance(audio) {
    ...
    brand.textContent = "Chroma Drift · stereo";
    ...
    function paint() {
      ...
      if (state.spec) {
        g.fillStyle = "#050506";
        g.fillRect(0, 0, w, h);
        const srcW = WINDOW_SEC / state.hopTime;
        let sx = t / state.hopTime - PLAYHEAD * srcW;
        let sw = srcW;
        let dx = 0;
        let dw = w;
        if (sx < 0) {
        ...
        if (sx + sw > state.spec.width) {
          sw = state.spec.width - sx;
          dw = (sw / srcW) * w;
        }
        g.imageSmoothingEnabled = false;
        if (sw > 0.5 && dw > 0.5) {
          g.drawImage(state.spec, sx, 0, sw, state.spec.height, dx, 0, dw, h);
        }
        drawOverlay(g, w, h);
        ...
        scrollAndDraw(g, w, h, playX, live.notesL, live.notesR);
      } else {
        g.fillStyle = "#050506";
        g.fillRect(0, 0, w, h);
        drawOverlay(g, w, h);
    ...
    function seekFromEvent(e) {
      ...
      const dt = (x / rect.width - PLAYHEAD) * WINDOW_SEC;
