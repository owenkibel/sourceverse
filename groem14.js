...
async function processSingleFile(inputFile, selectedPrompt) {
    ...
    try {
        ...
        const textApiPromises = textChunks.map((chunk, index) => {
            ...
            // Use the ORIGINAL chunk content for the API call, not the potentially trimmed one
            ...
            return openai.chat.completions.create({
                ...
                // Add more detail to the error object returned
                 return {
                     error: true,
                     chunkIndex: index + 1,
                     message: err.message,
        ...
        textApiResults.forEach((res, index) => {
            ...
            const imagePromptIndex = messageContent.indexOf("### Image Prompt");
            const videoPromptIndex = messageContent.indexOf("### Video Prompt");
            let verseEnd = messageContent.length;
            if (imagePromptIndex !== -1 && (videoPromptIndex === -1 || imagePromptIndex < videoPromptIndex)) {
                verseEnd = imagePromptIndex;
                sections.image = messageContent.substring(imagePromptIndex + 14).trim();
                if (videoPromptIndex !== -1) {
                    sections.image = sections.image.substring(0, sections.image.indexOf("### Video Prompt")).trim();
                    sections.video = messageContent.substring(videoPromptIndex + 15).trim();
                }
            } else if (videoPromptIndex !== -1) {
                verseEnd = videoPromptIndex;
                sections.video = messageContent.substring(videoPromptIndex + 15).trim();
                if (imagePromptIndex !== -1) {
                    sections.video = sections.video.substring(0, sections.video.indexOf("### Image Prompt")).trim();
                    sections.image = messageContent.substring(imagePromptIndex + 14).trim();
                }
            } else {
                sections.verse = messageContent;
            }
            sections.verse = sections.verse || messageContent.substring(0, verseEnd).trim();
