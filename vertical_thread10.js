...
async function buildNarrativeContext(domain, cumulativeModel) {
    ...
    context += `CRITICAL RUNTIME INSTRUCTION: Treat all historical forecasts below as structured benchmarks only. Do not reuse their language, metaphors, or phrasing in your new output.\n\n`;
    ...
    if (arc.forecastHistory && arc.forecastHistory.length > 0) {
        ...
        context += "--- RECENT STRUCTURED FORECAST BENCHMARKS ---\n";
        recentForecasts.forEach((f, i) => {
            context += `Act ${f.act} Forecast:\n${f.forecast}\n\n`;
        });

        injectedCount = recentForecasts.length;
        console.log(`   📡 Context injected: ${injectedCount} structured forecasts from narrative arc.`);
    }
