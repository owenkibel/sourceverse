...
const promptStateFile = path.join(path.dirname(process.argv[1]), 'grok.txt');

// Chunk Size Parameters
const maxChunkSize = 4096; // Global Variable for the chunk limit
const defaultChunkSize = 4096;

// Initialize OpenAI client with X.ai API key
...
function getNextPromptIndexSync() {
    try {
        const data = fss.readFileSync(promptStateFile, 'utf-8');
...
function setPromptIndexSync(index) {
    fss.writeFileSync(promptStateFile, String(index), 'utf-8')
}
...
async function processJsonFiles(directory) {
    ...
    for (file of files) {
        ...
        if (path.extname(filePath) === '.json') {
            try {
                let input = JSON.parse(await fs.readFile(filePath, 'utf8'));
                console.log(file);

                // Create a copy of the JSON file
                const jsonCopyPath = path.join(jsonDir, file);
                await fs.writeFile(jsonCopyPath, JSON.stringify(input, null, 2));

                if (input.hasOwnProperty('ogResult')) {
                    await processExistingJsonGrok(input, file);
...
async function processExistingJsonGrok(input, file) {
    console.log("Processing JSON (Grok):", input);
     ...
     let chunkSize = defaultChunkSize;
        ...
        const promises = [];
         async function processImage(imageUrl) {
            ...
            try {
                    ...
                    messages: [
                        {
                            ...
                            "content": [
                                ...
                                {
                                    "type": "text",
                                   "text": "Write a Shakespearean Sonnet about the image",
                                },
                            ],
                        },
                    ],
                ...
                return ""; // Handle image processing errors
            }
        }
         if (image) {
        ...
        for (let i = 0; i < words.length; i += chunkSize) {
            const chunk = words.slice(i, i + chunkSize).join(' ');
             const textProcessingPromise = openai.chat.completions.create({
                model: textModel,
                stream: false,
                temperature: 1,
                 messages: [
                   { role: "system", content: textTransformPrompt },
        ...
        const imageResult = image ? results[0] : ""; // Get image response if available
        const verseResponses = image ? results.slice(1) : results ;// Get verse responses based on whether there is an image
        const imageResponse = typeof imageResult === 'string' ? imageResult :  (image ? (typeof imageResult.choices !== "undefined" ? imageResult.choices[0].message.content : imageResult ) : '')
        ...
        verseResponses.forEach((res, index) => {
            ...
            const splitContent = res.choices[0].message.content.split("### Image Prompt");
            const verseContent = splitContent[0] || res.choices[0].message.content;
    ...
    ` : '');
         ...
         // Only remove excess line breaks - two will still be represented in markdown
...
title: ${input.ogResult.ogTitle.replace(/[^a-zA-Z0-9]/g, ' ')}-${textModel.replace(/[^a-zA-Z0-9]/g, '')}-${hash}
author: Grok
---
...
### Grok Models
<pre>Text Model: ${textModel}
<br> Vision Model: ${visionModel}</pre>
...
`;
        ...
        await fs.writeFile('posts/' + file.replace('.json', '') + '-' + textModel.replace(/[^a-zA-Z0-9]/g, '') + `-${hash}.md`, markdownOutput);

    } catch (error) {
        console.error(`Error processing ${file} (Grok):`, error);
    }
}
