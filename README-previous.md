...
Sourceverse is a collection of scripts for investigating text and media on the web and making creative text and media out of discernable threads in the input.

### Groetry

Groetry is the poetic output of Sourceverse as a Lume blog. It may be described as a light scrape with a dash of AI.


### Quickstart

generate-links1.js looks at recent Chrome Bookmarks and generates markdown output, finding related topics and creating a markdown post and a thread. Edit the local location of this file on the OS. Look for output in ./posts.

```
bun generate-links1.js
```
vertical_thread2.js is aimed at one thread at a time in ./x like this:

```
bun vertical_thread2.js --grok --grok-imagine --thread=t1
```

Apart from the API models from Google and X.ai, there are ways to process media in the scripts using installed binaries of ffmpeg and yt-dlp. A running instance of ComfyUI is also required for many of the media generations via module scripts that begin run_, such as run_kokoro_tts.js.
