...
const IMAGEN_MODEL_NAME = "imagen-4.0-ultra-generate-001"; // Default
const FLASH_IMAGE_MODEL_NAME = "gemini-2.5-flash-image-preview"; // Optional
...
let useFlashImageModel = false; // New flag for image model selection
let imageGenModelInUse = IMAGEN_MODEL_NAME; // Variable to track the current image model
...
try {
    ...
    console.log(`Image Gen will default to ${IMAGEN_MODEL_NAME}. Use --use-flash-image for ${FLASH_IMAGE_MODEL_NAME}.`);
    console.log(`Video Gen will use ${VEO_3_MODEL_NAME}.`);
} catch (modelError) {
...
async function generateAndEmbedVideo(videoPromptContent, baseFilename, imageInput = null) {
    ...
    if (veo3Result.success) {
        ...
        console.error(`Veo 3 generation failed: ${veo3Result.error}.`);
        veo3Result.modelUsed = VEO_3_MODEL_NAME; // Still report the model that was attempted
    }
    return veo3Result;
}

async function generateAndEmbedImageImagen(imagePromptContent, baseFilename) {
    if (!googleGenAIClient) {
    ...
    console.log(`Generating image with prompt: "${trimmedPrompt.substring(0, 150)}..." (using ${IMAGEN_MODEL_NAME})`);
    try {
        ...
        if (!apiResponse?.generatedImages?.length) {
            ...
            return { success: false, error: errorMsg, markdown: `<!-- ${escapeHtml(errorMsg)} -->`, refusalReason, detailedErrorInfo: { type: "NoImagesReturned", apiResponse: apiResponse || null } };
        }
        ...
        if (!generatedImage.image?.imageBytes) {
            ...
            return { success: false, error: errorMsg, markdown: `<!-- ${escapeHtml(errorMsg)} -->`, refusalReason: errorMsg, detailedErrorInfo: { type: "NoImageBytesInResponse", generatedImage } };
        }
...
async function generateAndEmbedImageFlash(imagePromptContent, baseFilename) {
    if (!googleGenAIClient) {
        return { success: false, error: "Image generation client not initialized.", markdown: "<!-- Image generation skipped: Client not initialized -->" };
    }
    const trimmedPrompt = imagePromptContent.trim();
    const promptWithAspectRatio = `${trimmedPrompt}\n\nImportant: Generate this image with a cinematic 16:9 aspect ratio.`;
    console.log(`Generating image with prompt: "${promptWithAspectRatio.substring(0, 150)}..." (using ${FLASH_IMAGE_MODEL_NAME})`);
    try {
        const apiResponse = await googleGenAIClient.models.generateContent({
            model: FLASH_IMAGE_MODEL_NAME,
...
async function generateAndEmbedImage(imagePromptContent, baseFilename) {
    if (!imagePromptContent?.trim()) {
         return { success: false, error: "No valid prompt provided for image generation.", markdown: "<!-- Image generation skipped: No valid prompt provided -->" };
    }
    if (useFlashImageModel) {
        imageGenModelInUse = FLASH_IMAGE_MODEL_NAME;
        return await generateAndEmbedImageFlash(imagePromptContent, baseFilename);
    }
    imageGenModelInUse = IMAGEN_MODEL_NAME;
    return await generateAndEmbedImageImagen(imagePromptContent, baseFilename);
}
    ...
    try {
        ...
        if (extractedVideoPrompts.length > 0) {
            ...
            if (videoGenRes.success) {
                ...
                generatedVideoOutput = `### Generated Video\n<p><strong>Video Generation Failed.</strong></p><p><em>Error Summary:</em> ${escapeHtml(videoGenRes.error || 'Unknown error')}</p>\n<p><em>Attempted prompt:</em></p>\n<pre><code class="language-text">${escapeHtml(promptUsedForVideo)}</code></pre>`;
            }
        }
...
<p><strong>Text:</strong> ${TEXT_MODEL_NAME} (temp: 1)<br><strong>Vision:</strong> ${VISION_MODEL_NAME}<br><strong>Image Gen:</strong> ${imageGenModelInUse}<br><strong>TTS:</strong> ${TTS_MODEL_DISPLAY_NAME}<br><strong>Video:</strong> ${videoModelUsed}</p>
...
async function main() {
    ...
    if (process.argv.includes('--use-flash-image')) {
        useFlashImageModel = true;
        console.log(">> Using optional Flash image model (--use-flash-image).");
    }
    
    if (process.argv.includes('--text-only-video')) {
